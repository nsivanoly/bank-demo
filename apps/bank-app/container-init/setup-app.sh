...
APP_NAME=${APP_NAME:-BankingFrontendAppReact}
APP_DESC=${APP_DESC:-"Banking Frontend Single Page Application React"}
FRONTEND_URL=${FRONTEND_URL:-https://localhost:3000}
CALLBACK_URL=${CALLBACK_URL:-${FRONTEND_URL}}
LOGOUT_URL=${LOGOUT_URL:-${FRONTEND_URL}}
...
CONFIG_FILE=${CONFIG_FILE:-../src/config.json}
...
log_info() { echo "ℹ️  [BANK-APP-SETUP] $*" >&2; }
log_success() { echo "✅ [BANK-APP-SETUP] $*" >&2; }
log_error() { echo "❌ [BANK-APP-SETUP] $*" >&2; }
log_warn() { echo "⚠️  [BANK-APP-SETUP] $*" >&2; }
...
update_config_json() {
  ...
  log_info "Updating config.json with client credentials"
  ...
  # Update config using sed with safer approach
  [ -n "$is_client_id" ] && {
    log_info "Updating WSO2_IS clientID: ${is_client_id:0:15}..."
    sed -i.tmp "/\"WSO2_IS\"/,/^  }/ s/\"clientID\": \"[^\"]*\"/\"clientID\": \"$(echo "$is_client_id" | escape_sed)\"/" "$CONFIG_FILE"
  }
  
  [ -n "$km_client_id" ] && {
    log_info "Updating WSO2_IS_KM clientID: ${km_client_id:0:15}..."
    sed -i.tmp "/\"WSO2_IS_KM\"/,/^  }/ s/\"clientID\": \"[^\"]*\"/\"clientID\": \"$(echo "$km_client_id" | escape_sed)\"/" "$CONFIG_FILE"
  }
  
  [ -n "$apim_km_client_id" ] && {
    log_info "Updating WSO2_APIM_KM clientID: ${apim_km_client_id:0:15}..."
    sed -i.tmp "/\"WSO2_APIM_KM\"/,/^  }/ s/\"clientID\": \"[^\"]*\"/\"clientID\": \"$(echo "$apim_km_client_id" | escape_sed)\"/" "$CONFIG_FILE"
  }
...
main() {
  ...
  log_info "========================================="
  update_config_json "$is_client_id" "$is_client_id" "$apim_km_client_id"
